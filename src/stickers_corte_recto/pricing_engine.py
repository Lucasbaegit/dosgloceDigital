...
class StickersCorteRectoPricingEngine:
    ...
    def __init__(self, bundle: StickersCorteRectoBundle, project_root: Any | None = None):
        ...
        self._formula_config_path = (
            self._project_root / "data" / "stickers_corte_recto" / "formula_editable_config.json"
    ...
    def quote(self, request: StickersCorteRectoQuoteInput) -> StickersCorteRectoQuoteResult:
        ...
        # El precio comercial final se mantiene igual al PDF/lista; las variables cambian la base tecnica.
        total = total_pdf
        ...
        trace = build_pdf_matrix_trace(
            ...
            fuente_precio_final="PDF pagina 15 - Stickers Corte Recto",
            ...
            extras={
                ...
                "formula_excel_reconstruida": "((material_base + click_color_base * coeficiente_formato) * laca_uv_factor * corte_recto_factor * coeficiente_cantidad) * multiplicador_comercial",
                "config_variables_base": self._variables_bundle.config_variables_base if self._variables_bundle else None,
                "config_formula_editable_path": "data/stickers_corte_recto/formula_editable_config.json",
                "variables_principales_usadas": [
                    {"key": "factor_laca_uv_stickers_corte_recto", "label": "Factor Laca UV Stickers Corte Recto", "value": formula_breakdown["laca_factor"], "unit": "factor"},
                    {"key": "corte_recto_factor_stickers_corte_recto", "label": "Factor corte recto Stickers Corte Recto", "value": formula_breakdown["corte_factor"], "unit": "factor"},
                    {"key": "multiplicador_comercial_stickers_corte_recto", "label": "Multiplicador comercial Stickers Corte Recto", "value": formula_breakdown["multiplicador_comercial"], "unit": "factor"},
                    {"key": f"coeficiente_formato_stickers_corte_recto_{self._safe_key(request.formato)}", "label": f"Coeficiente formato Stickers Corte Recto {request.formato}", "value": formula_breakdown["coeficiente_formato"], "unit": "factor"},
                    {"key": f"coeficiente_cantidad_stickers_corte_recto_{request.cantidad_unidades}", "label": f"Coeficiente cantidad Stickers Corte Recto {request.cantidad_unidades}", "value": formula_breakdown["coeficiente_cantidad"], "unit": "factor"},
                ],
            },
        )
        return StickersCorteRectoQuoteResult(
            ...
            regla_aplicada=(
                "STICKERS_CORTE_RECTO_FORMULA_EDITABLE_CALIBRADA_P15"
                if modo_precio == "formula_editable_calibrada"
                else "STICKERS_CORTE_RECTO_MATRIZ_PDF_P15"
            ),
    ...
    def _compute_formula_breakdown(self, request: StickersCorteRectoQuoteInput, overrides: dict[str, Any], formula_config: dict[str, Any]) -> dict[str, float]:
        ...
        material_base = float(cfg.get("material_base", 0.0))
        ...
        if "factor_laca_uv_stickers_corte_recto" in overrides:
            laca_uv_factor = float(overrides["factor_laca_uv_stickers_corte_recto"])
        if "corte_recto_factor_stickers_corte_recto" in overrides:
            corte_recto_factor = float(overrides["corte_recto_factor_stickers_corte_recto"])
        if "multiplicador_comercial_stickers_corte_recto" in overrides:
            multiplicador_comercial = float(overrides["multiplicador_comercial_stickers_corte_recto"])
        format_override_key = f"coeficiente_formato_stickers_corte_recto_{self._safe_key(request.formato)}"
        quantity_override_key = f"coeficiente_cantidad_stickers_corte_recto_{request.cantidad_unidades}"
        ...
        subtotal = (material_base + click_total) * laca_factor * corte_recto_factor * coeficiente_cantidad
